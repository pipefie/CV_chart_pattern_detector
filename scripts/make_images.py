...
def split_tag(ts_end: pd.Timestamp, wf_cfg: dict) -> str:
    # Use your walk_forward dates to decide bucket by the window's END timestamp
    ...
    if ts_end <= train_end:
        return "train"
    elif ts_end <= validate_end:
        return "val"
    elif ts_end <= test_end:
        return "test"
    else:
        return "ignore"
...
def render_window_png(
    ...
    # 1) Make a style with string colors only (no RGBA tuples here)
    style = mpf.make_mpf_style(
        base_mpl_style="seaborn-v0_8",
        gridstyle="-" if grid_on else "",
        gridcolor="#000000",           # <- string, not tuple
        facecolor=bg,
        edgecolor=bg,
    ...
    fig = None
    try:
        # Try wick line width variants (for mplfinance version differences)
        try:
        ...
        # axes can be a single Axes or a list/array; normalize:
        # Normalize axes
        ax_main = axes[0] if isinstance(axes, (list, tuple, np.ndarray)) else axes

        # Ticks visibility (use the main panel)
        if not show_ticks:
            ax_main.set_xticks([])
            ax_main.set_yticks([])

        # Read limits for metadata from the main panel
        x_min, x_max = ax_main.get_xlim()
        y_min, y_max = ax_main.get_ylim()

        # If grid is on, adjust alpha *for all axes that have gridlines*
        if grid_on:
            axes_iter = axes if isinstance(axes, (list, tuple, np.ndarray)) else [axes]
            for ax in axes_iter:
                # Some mplfinance versions expose these methods; otherwise skip gracefully
                try:
    ...
    # 4) Postprocess jitter: blur & noise
    ...
    try:
        ...
        if noise_std and float(noise_std) > 0:
            # Make numpy RNG deterministic from parent rng
    ...
    for group, ucfg in universes.items():
        ...
        for sym in ucfg["symbols"]:
            ...
            for i in range(args.window_bars, len(df)+1, args.stride_bars):
                ...
                # Naming from render.yaml
                schema = render_cfg["export"]["naming"]["schema"]
                # You can add a simple seed component to vary jitter deterministically
                ...
                render_window_png(
                    win, out_png, meta_out, render_cfg,
                    symbol=sym, timeframe=timeframe, rng=rng
                )
                # periodic cleanup to avoid handle leaks
                counter += 1
                if (counter % 50) == 0:
                    plt.close("all")
                    gc.collect()
